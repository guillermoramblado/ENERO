...
EPISODE_LENGTH = 100
NUM_ACTIONS = 100
percentage_demands = 50 # Percentage of demands that will be used in the optimization
...
nombre_carpetas_topologias = ["NEW_EliBackbone","NEW_HurricaneElectric","NEW_Janetbackbone"]
...
nombre_grafos_topologias = ["EliBackbone","HurricaneElectric","Janetbackbone"]
...
def aplicar_hill_climbing(tm_id, best_routing, list_of_demands_to_change, nombre_grafo_topologia, ruta_carpeta_topologia):
    ...
    while 1:
        ...
        #Obtenemos nextVal: uso del enlace de mayor uso aplicando la acción indicada en next_state (dicho uso en NEGATIVO), mientras que next_state es una tupla (accion,origen demanda, destino demanda) seleccionada en la exploración
        nextVal, next_state = hill_climb_agent.explore_neighbourhood_DRL_sp(env_hill_climb)
        #Criterio de parada del algoritmo: cuando no podemos seguir mejorando, o cuando la diferencia de mejora es minúscula. También se podría imponer un límite de iteraciones (exploraciones)
...
if __name__ == "__main__":
    ...
    print("\nGenerando entornos de validación...")
    entornos = []
    ...
    #Abrimos el log que usaremos para controlar que todo va correctamente
    ruta_log = "./Logs/DRL_vs_ILP.txt"
    modo_apertura = ''
    if not os.path.exists(ruta_log):
        modo_apertura = 'w'
    else:
        modo_apertura = 'a'
    fileLogs = open(ruta_log,modo_apertura)

    for env in entornos:
        fileLogs.write(f'----------- TOPOLOGIA {env.graph_topology_name} ----------\n')
        problema = ILP.ProblemaProgramacionLineal(env.dataset_folder_name)
        for tm_id in range(50):
            fileLogs.write(f'\nProcesando matriz de tráfico {tm_id}')
            inicio_matriz = tt.time()
            ...
            for (clave,valor) in zip(datos,["DRL",env.graph_topology_name,round(uso_final,4),round(tiempo_DRL,4)]):
            ...
            for (clave,valor) in zip(datos,["DRL+LS",env.graph_topology_name,round(uso_final,4),round(tiempo_DRL+tiempo_LS,4)]):
                datos[clave].append(valor)
            
            #Ahora resuelvo usando programación lineal
            uso, tiempo = problema.resolver_matriz_trafico(tm_id)
            for (clave,valor) in zip(datos,["ILP",env.graph_topology_name,round(uso,4),round(tiempo,4)]):
                datos[clave].append(valor)

            fin_matriz = tt.time()

            fileLogs.write(f' ----> Finalizado en {round(fin_matriz-inicio_matriz,4)} segundos')
            fileLogs.flush()
    ...
    print("Creando los archivos CSV correspondientes...")
    ...
    csv_resultados = ruta_dir_DRL_ILP + "/resultados_DRL_vs_ILP.csv"
    if not os.path.exists(ruta_dir_DRL_ILP):
        os.makedirs(ruta_dir_DRL_ILP)
    df = pd.DataFrame(datos)
    if not os.path.exists(csv_resultados):
        df.to_csv(csv_resultados,header=True,index=False,mode='w')
    else:
        df.to_csv(csv_resultados,header=False,index=False,mode='a')

    df = pd.read_csv(ruta_dir_DRL_ILP+"/resultados_DRL_vs_ILP.csv")
    medias = df.groupby(["Modelo", "Topologia"]).agg({"Tiempo": "mean","Uso": "mean"})
    medias.to_csv(ruta_dir_DRL_ILP+"/medias_tiempo_uso.csv")
