...
#Definimos los diferentes porcentajes de demandas con los que queremos probar el rendimiento de ENERO
porcentajes_demandas = [15,30,50]
porcentajes_demandas = [x/100 for x in porcentajes_demandas]

#Método encargado de generar un entorno, siendo necesario especificar la topología de trabajo y el porcentaje de demandas con las que se quiere trabajar
#Este método ofrece la posibilidad de trabajar con cualquier de las matrices de validación
def generar_entorno_validacion(carpeta_topologia,grafo_topologia,porcentaje_demandas):
    env_eval = gym.make(ENV_NAME)
    env_eval.seed(SEED)
    env_eval.generate_environment(carpeta_topologia+"/EVALUATE", grafo_topologia, EPISODE_LENGTH, NUM_ACTIONS, porcentaje_demandas)
...
def obtener_modelo_base_comprimido():
    ...
    checkpoint.restore("../DECORE/ApagadoAleatorio/apagado_30%").expect_partial()
    #Aplico la compresion final
    actor_comprimido = ModeloBaseComprimido(actor_mascaras,tiene_agentes=False)
    actor_comprimido.build()
    #input(f'Comprimido el modelo:{actor_comprimido.count_params()}')
...
'''
def aplicar_hill_climbing(tm_id, best_routing, list_of_demands_to_change, nombre_grafo_topologia, ruta_carpeta_topologia, porcentaje_demandas):
    ...
    env_hill_climb.generate_environment(ruta_carpeta_topologia, nombre_grafo_topologia, EPISODE_LENGTH_MIDDROUT, NUM_ACTIONS, porcentaje_demandas)
    ...
    start = tt.time()
    #Definimos un contador, para contar el número de iteraciones que hace
    contador = 0
    while 1:
    ...
    return currentVal*(-1), end-start, contador
...
if __name__ == "__main__":
    ...
    #Creo un log donde iré guardando el progreso 
    fileLogs = open("./Logs/porcentajes_demandas.txt", "w")

    print("\nComenzando el proceso de comparación...")
    for porcentaje in porcentajes_demandas:
        fileLogs.write(f'------------> PORCENTAJE DE DEMANDAS SELECCIONADO: {porcentaje*100}%')
        #Aplicamos ENERO sobre las topologías indicadas
        for (ruta_top, grafo_top) in zip(ruta_carpetas_topologias,nombre_grafos_topologias):
            #Creamos un entorno para dicha topología y con dicho porcentaje de demandas seleccionadas
            env = generar_entorno_validacion(ruta_top,grafo_top,porcentaje)
            fileLogs.write(f'\n\nCAMBIANDO A TOPOLOGÍA {env.graph_topology_name}')
            for tm_id in range(50):
                fileLogs.write(f'\nMatriz de tráfico {tm_id}')
                fileLogs.flush()
                ...
                uso_final, tiempo_LS, iters = aplicar_hill_climbing(tm_id,mejor_enrutamiento,lista_demandas_a_cambiar,env.graph_topology_name,env.dataset_folder_name,porcentaje)
                #Almacenamos en el diccionario los resultados
                for (clave,valor) in zip(datos,[f'{int(porcentaje * 100)}%',env.graph_topology_name,np.round(uso_final,4),np.round(tiempo_DRL+tiempo_LS,4)]):
                    datos[clave].append(valor)
        #Pasamos a trabajar con un % distinto de demandas
        fileLogs.write("\n\n")
        fileLogs.flush()

    #Salimos del 'for' principal
    fileLogs.close()
    ...
    ruta_porcentajes_demandas = "./Imagenes/PorcentajesDemandas"
    if not os.path.exists(ruta_porcentajes_demandas):
        os.makedirs(ruta_porcentajes_demandas)
    df = pd.DataFrame(datos)
    ruta_csv = ruta_porcentajes_demandas+"/resultados_comparacion_porcentajes.csv"
    df.to_csv(ruta_csv, mode='w', header=True, index=False)

    df = pd.read_csv(ruta_csv)
    medias = df.groupby(["Modelo", "Topologia"]).agg({"Tiempo": "mean","Uso": "mean"})
    medias.to_csv(ruta_porcentajes_demandas+"/medias_tiempo_uso.csv")

    print("Creando los boxplots correspondientes...")
    ...
    for valor, nombre, etiqueta in zip(valores_graficar,nombre_graficos,etiquetas_ejeY):
        ...
        plt.title(r"Comparativa ENERO por % de demandas críticas")
        plt.ylabel(etiqueta)
        plt.xlabel("Topologia")
        plt.legend(title="Modelo", loc="upper left")

        plt.tight_layout()
        path_imagen_boxplots = ruta_porcentajes_demandas + "/" + nombre
